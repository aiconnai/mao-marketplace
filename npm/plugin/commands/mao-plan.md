...
## Step 0: Research (brownfield / unfamiliar APIs)

If the task involves an existing codebase or unfamiliar APIs/libraries, first spawn
a research agent (sonnet) to produce `.orchestrator/state/context.md`:

- Architecture scan: project type, framework, conventions, directory structure
- Relevance scan: existing code related to the task, files likely to change
- Stack research: API docs, common patterns, pitfalls (only if unfamiliar tech)

Skip this step for greenfield projects with well-known patterns.

## Step 1: Understand the Codebase

Before decomposing:
1. Read `.orchestrator/state/context.md` if the research step produced one
2. Read the project's CLAUDE.md (if it exists) for conventions
3. Search the codebase for existing patterns related to the request
...
Override rules (these always apply regardless of score):
- Decomposition → always opus
- Verification (tests/lint) → always haiku
- Code review → always sonnet
- Meta-reflection → always opus
